...
function Sidebar({ currentName, timeRange, setTimeRange, onExportClick }) {
  ...
  const numberOfPosts = formatNumber(postInfo.number * getRandomMultiplier());
    ...
    <div className='text-lg mt-2'>Number of Subscribers: {formatNumber(postInfo.number)}</div>
    <div className='text-lg mt-2'>Number of Posts: {numberOfPosts}</div>
    <div className='text-lg mt-2'>Currently Online: {currentOnline}</div>
