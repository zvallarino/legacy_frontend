...
function Rauthsearch() {
  // Separate state for last name and first name.
  ...
  const exportToExcel = () => {
    ...
    // Generate a filename using the names and current date.
    const currentDate = new Date();
    const formattedDate = `${String(currentDate.getMonth() + 1).padStart(2, '0')}-${String(currentDate.getDate()).padStart(2, '0')}-${currentDate.getFullYear()}`;
    const queryLabel = firstName ? `${lastName} ${firstName}` : lastName;
    const filename = `${queryLabel} ${formattedDate}.xlsx`;
  ...
  const searchPosts = async () => {
    ...
    try {
      // Build the URL with separate query parameters for last name and first name.
      const url = `http://127.0.0.1:8000/search_app/nih_research_author/?last_name=${encodeURIComponent(lastName)}&first_name=${encodeURIComponent(firstName)}`;
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Require last name input for the search.
    if (lastName) {
      searchPosts();
    }
  };
    ...
    <div>
      <div className="flex justify-center">
        ...
        <form onSubmit={handleSubmit} className="flex items-center border-2 border-gray-300 rounded-lg">
          {/* Input for last name (required) */}
          <input
            type="text"
            value={lastName}
            onChange={(e) => setLastName(e.target.value)}
            placeholder="Last Name"
            className="w-48 h-12 px-4 text-lg"
          />
          {/* Input for first name (optional) */}
          <input
            type="text"
            value={firstName}
